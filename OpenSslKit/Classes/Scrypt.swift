...
//
// This file was copied from TrustKeystore and modified by changing cryptoSwift library on OpenSsl.
...
public final class Scrypt {
    ...
    public func calculate(password: String) throws -> Data {
        ...
        let result = try scrypt(password: passwordData, salt: params.salt)
        return Data(result)
    }

    /// Runs the key derivation function with a specific password.
    public func calculate(data: Data) throws -> Data {
        if let error = params.validate() {
            throw error
        }

        let result = try scrypt(password: data, salt: params.salt)
        return Data(result.reversed())
    }

    /// Computes scrypt.
    private func scrypt(password: Data, salt: Data) throws -> Data {
        // Allocate memory.
        ...
        let barray = Kit.hmacpbfdf2(pass: password, salt: salt, length: params.p * 128 * params.r, iterations: 1)
        ...
        let block = Data(bufferPointer)

        return Kit.hmacpbfdf2(pass: password, salt: block, length: params.desiredKeyLength, iterations: 1)
    }
